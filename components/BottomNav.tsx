...
const BottomNav: React.FC<Props> = ({ activeTab, onTabChange, onOpenProfile, user }) => {
  ...
  return (
    <nav className="fixed bottom-0 left-0 w-full bg-white dark:bg-background-dark border-t border-gray-100 dark:border-white/5 px-6 pb-6 pt-2 flex justify-between items-end text-gray-400 dark:text-gray-500 z-50">
      {navItems.map((item) => {
        if (item.id === 'fab') {
            return (
